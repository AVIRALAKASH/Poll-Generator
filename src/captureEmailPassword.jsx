...
async function captureEmailPassword( email, password, mint ) {
	...
	let success, message, gotoSignin = false;
	if ( mint ) {
		try {
			...
			await db.collection( "users" ).doc( uid ).set( { __uid, uid, name, email, profile: `https://www.gravatar.com/avatar/${ md5 }?s=200&d=identicon`, polls: [], votes: [], isComplete: true,	isAdmin: false } );
		} catch ( error ) {
			success = false;
	...
	return { success, message, gotoSignin };
}