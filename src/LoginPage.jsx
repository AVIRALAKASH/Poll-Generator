...
`;
const or = <span style={ { color: "rgba( 0, 0, 0, .4 )" } }>or </span>

class App extends React.Component {
	static email = /^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$/i;
	static password = /^.{12,}$/;
	static hint = "( 12+ characters )";
	static defaultMint = parseUrl( window.document.location ).search[ "mint" ] !== "false";
	static errors = [
		"Fill all the boxes.",
		"Enter valid email.",
		"Passwords must be atleast 12 characters.",
	...
	switchMint( value ) {
		this.setState( { mint: !!value, message: null, error: false } );
	...
	invalidSubmit() {
		const { textEmail: e, textPassword: p, textConfirm: c } = this.state;
		let error = 0;
		( p !== c ) && ( error = 3 );
		( !App.password.test( p ) ) && ( error = 2 );
		( !App.email.test( e ) ) && ( error = 1 );
		( e === "" || p === "" || c === "" ) && ( error = 0 );
		this.setState( { message: App.errors[ error ], error: true } );
	}
	handleChange( event ) {
		this.setState( { [ "text" + event.target.name.slice( 6 ) ]: event.target.value } );
	}
	async respond( mintButton ) {
		this.setState( { message: "working...", error: false } );
		const response = await captureEmailPassword( this.state.textEmail, this.state.textPassword, mintButton );
		if ( response === null ) {
			this.setState( { message: "Unknown error occurred.", error: true } );
			return;
		}
		mintButton && response.gotoSignin && this.switchSignIn();
		this.setState( { message: response.message || null, error: !response.success } );
		if ( mintButton === true || response.success !== true ) {
			this.setState( { textPassword: "", textConfirm: "" } );
		}
		if ( mintButton ) {
	...
	render() {
		...
		return (
			<Container className={ this.state.mint ? "mint" : null }>
				<SignUp className="sign-up">
					<Title className="title" onClick={ this.switchSignUp }>{ this.state.mint ? false : or }Sign Up</Title>
					<Form className="form">
						<input type="text" ref={ this.signupEmail } className={ emailClass } name="signupEmail" placeholder="e-mail" value={ this.state.textEmail } onChange={ this.handleChange }/>
						<input type="password" className={ passwordClass } name="signupPassword" placeholder={ "password " + App.hint } value={ this.state.textPassword } onChange={ this.handleChange }/>
						<input type="password" className={ confirmClass } name="signupConfirm" placeholder="confirm password" value={ this.state.textConfirm } onChange={ this.handleChange }/>
						<button type="submit" className={ signupValidity ? "valid" : null } onClick={ () => signupValidity ? this.respond( true ) : this.invalidSubmit() }>sign-up</button>
					</Form>
				</SignUp>
				<SignIn className="sign-in">
					<Title className="title" onClick={ this.switchSignIn }>{ this.state.mint ? or : false }Sign In</Title>
					<Form className="form">
						<input type="text" ref={ this.signinEmail } className={ emailClass } name="signinEmail" placeholder="e-mail" value={ this.state.textEmail } onChange={ this.handleChange }/>
						<input type="password" ref={ this.signinPassword } className={ passwordClass } name="signinPassword" placeholder={ "password " + App.hint } value={ this.state.textPassword } onChange={ this.handleChange }/>
						<button type="submit" className={ signinValidity ? "valid" : null } onClick={ () => signinValidity ? this.respond( false ) : this.invalidSubmit() }>sign-in</button>
					</Form>
				</SignIn>
				<Message className={ "message" + ( this.state.error ? " error" : "" ) }>{ this.state.message }</Message>
			</Container>
		);
	}
}
